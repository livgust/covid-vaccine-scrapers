...
if (process.env.NODE_ENV === "production" && USE_TWITTER) {
    ...
    console.log(
        `SETTING UP FAKE TWITTER CLIENT - If you intended this to be real, set NODE_ENV to "production" instead of ${process.env.NODE_ENV}.`
    );
    client = {
        post: (method, args) => {
            console.log(
                `Twitter client was called with ${JSON.stringify([
                    method,
                    args,

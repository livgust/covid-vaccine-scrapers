...
async function writeScrapedData({
    ...
    for (const item of individualLocationData) {
        ...
        if (
            hasAvailability &&
            // there is at least one sign-up link somewhere
            Object.values(availability || []).reduce(
                (acc, cur) => acc || cur.signUpLink,
                signUpLink
            )
        ) {
    ...
    const mappedLocations = queryResults.parentLocations.map(
        (parentLocationEntry) =>
            parentLocationEntry.locations.map((locationEntry) => ({
                    ...
                    (obj, entry) => {
                        ...
                        if (!obj[apptDate]) {
                            obj[apptDate] = {
                                ...
                                extraData: entry.data.extraData, // TODO: check on this
                            };
                        } else {
                            obj[apptDate] = {
                                ...obj[apptDate],
                ...
                restrictions: "TODO", // TODO: check on this
                extraData: {},
            }))
    );
    let locationList = [];

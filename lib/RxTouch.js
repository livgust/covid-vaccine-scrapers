...
async function GetFinalAvailability(page, facilityId, siteName) {
    ...
    const finalAvailability = dailyAvailability.flat().reduce((acc, entry) => {
    ...
    return {
        message:
            Object.keys(finalAvailability).length != 0
                ? `Availability found. Search on signup website for zip ${loc.zip}`
                : `No available appointments for this facility`,
        ...
        if (!results[loc.zip]) {
            ...
            if ((await page.url()) != scheduleUrl) {
                ...
                if ((await page.url()) === patientAdvisoryUrl) {
                    ...
                    results[loc.zip] = {
                        message: `Stuck in queue-it system, can't determine availability.`,
                        availability: {},
                    };
                }
            ...
            if (!facilityId) {
                results[loc.zip] = {
                    message: "No available appointments - no facility found.",
                    availability: {},
                };
                continue;
            }
            ...
            results[loc.zip] = await GetFinalAvailability(
                page,
                facilityId,
                siteName
            );

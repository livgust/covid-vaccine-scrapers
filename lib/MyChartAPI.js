...
function UpdateResults(results, postResponse) {
    ...
    allLocations = postResponse.AllDepartments;

    if (!allLocations) {
        console.warn(`No data found in postResponse, error code: ${postResponse.ErrorCode}`)
        return results
    }
    ...
    allDays = Object.values(postResponse.AllDays);

const { toTitleCase } = require("./stringUtil");
...
function AddSiteInfo(results, defaultResults, siteKey, siteInfo) {
    ...
    results[siteKey].street = toTitleCase(Address.Street.join(" "));
    results[siteKey].city = toTitleCase(Address.City);
...
function UpdateResults(results, postResponse) {
    ...
    for (const dayEntry of allDays) {
        ...
        if (date.isSameOrAfter(midnightToday)) {
            results[key].availability[date.format("M/D/YYYY")] = {

...
(process.env.FAUNA_DB ? describe : describe.skip)(
    ...
    () => {
        it("creates parent location if it doesn't exist", async () => {
            ...
            expect(
                parentLocationRefId,
                "an entry is created and an ID is returned"
            ).to.exist;
...
(process.env.FAUNA_DB ? describe : describe.skip)(
    ...
    () => {
        it("retrieves successfully", async () => {
            if (
                await dbUtils.checkItemsExistByRefIds("locations", ["123"])[0]
            ) {
                ...
                (
                    ...
                    return {
                        scraperRun: {
                            ...
                            data: {
                                locationRefId:
                                    entry.scraperRun.data.locationRef.id,
                                parentScraperRunRefId:
                                    entry.scraperRun.data.parentScraperRunRef
                                        .id,
                            },
                        },
...
(process.env.FAUNA_DB ? describe : describe.skip)(
    ...
    () => {
        it("writes & reads", async () => {
            ...
            expect(
                res.results.find(
                    (res) => res.name === `RandomName-${randomName}`
                )
            ).to.shallowDeepEqual({
                name: `RandomName-${randomName}`,
                street: "2240 Iyannough Road",
